package com.smartmarket.api.controllers.impl;

import com.smartmarket.api.annotations.PageableAsQueryParam;
import com.smartmarket.api.controllers.IController;
import com.smartmarket.api.mappers.UserMapper;
import com.smartmarket.api.models.entities.User;
import com.smartmarket.api.models.security.RegisterRequest;
import com.smartmarket.api.services.UserService;
...
public class UserController implements IController<User, RegisterRequest> {
    ...
    @PreAuthorize("hasRole('ROLE_ADMIN') or authentication.principal.id == #id")
    public Mono<Void> update(@PathVariable UUID id, @RequestBody @Valid RegisterRequest registerRequest) {
        User user = UserMapper.INSTANCE.toUser(registerRequest);
