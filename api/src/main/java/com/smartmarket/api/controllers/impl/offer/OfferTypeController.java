...
public class OfferTypeController implements IController<OfferType, OfferTypeDTO> {
    ...
    @Override
    @PostMapping
    public Mono<OfferType> save(@RequestBody @Valid OfferTypeDTO offerTypeDTO) {
    ...
    @Override
    @PutMapping("/{id}")
    public Mono<Void> update(@PathVariable UUID id, @RequestBody @Valid OfferTypeDTO offerTypeDTO) {
        OfferType offerType = OfferTypeMapper.INSTANCE.toOfferType(offerTypeDTO);
        offerType.setId(id);
