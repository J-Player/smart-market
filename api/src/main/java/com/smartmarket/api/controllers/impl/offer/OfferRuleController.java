...
public class OfferRuleController implements IController<OfferRule, OfferRuleDTO> {
    ...
    @Override
    @PostMapping
    public Mono<OfferRule> save(@RequestBody @Valid OfferRuleDTO offerRuleDTO) {
    ...
    @Override
    @PutMapping("/{id}")
    public Mono<Void> update(@PathVariable UUID id, @RequestBody @Valid OfferRuleDTO offerRuleDTO) {
        OfferRule offerRule = OfferRuleMapper.INSTANCE.toOfferRule(offerRuleDTO);
        offerRule.setId(id);
