...
public class MarketController implements IController<Market, MarketDTO> {
    ...
    @GetMapping("/all")
    public Flux<Market> findAll() {
        return marketService.findAll();
    }

    @Override
    @PostMapping
    public Mono<Market> save(@RequestBody @Valid MarketDTO marketDTO) {
    ...
    @Override
    @PutMapping("/{id}")
    public Mono<Void> update(@PathVariable UUID id, @RequestBody @Valid MarketDTO marketDTO) {
        Market market = MarketMapper.INSTANCE.toMarket(marketDTO);
        market.setId(id);
