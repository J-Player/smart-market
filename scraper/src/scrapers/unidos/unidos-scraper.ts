...
export class UnidosScraper extends MarketScraper {
	...
	protected override async _start() {
		...
		const $dateTime = await page.waitForSelector(XPATHS.validate, { timeout: 30000 })
		const dateTime = await $dateTime!.evaluate(el => el.getAttribute('data-countdown')!)
		await page.waitForSelector(XPATHS.products, { timeout: 30000 })
	...
	private async getProduct(
		...
		return callback({
			...
			offers: [{ price: parseNumber(offer_price) }]
		})
	}
}
