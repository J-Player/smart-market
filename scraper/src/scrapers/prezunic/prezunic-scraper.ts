...
export class PrezunicScraper extends MarketScraper {
	...
	protected override async _start() {
		...
		if ($popup) {
			await Promise.all([$popup.click(), this.logger.debug('Popup fechado')])
		}
		...
		for (let i = 1; i <= total; i++) {
			...
			await Promise.all([page.waitForNavigation(), buttonShowMore!.click()])
	...
	private async getProduct(element: ElementHandle<Element>): Promise<Product | undefined> {
		const XPATHS = {
			...
			product_price: xpath(
				'.//*[contains(@class, "currencyContainer--summary")]//span[contains(@class, "currencyContainer")]'
			),
		...
		const prices = await element
			.$$eval(XPATHS.product_price, el =>
				el.map(el => {
		...
		const originalPrice = weight
			? convertPriceToKg(prices[0], parseNumber(weight.value), weight.unitMeasure)
			: prices[0]
		...
		if (originalPrice !== 0) {
			offers.push({ type: $offer_club ? PrezunicOfferType.CLUBE : undefined, price: discountPrice })
		}
		...
		if (offers.length > 0) {
			...
			if (hasSpecialOffer) {
				const promotions = await element.evaluate(
					() => JSON.parse(localStorage.getItem('promotions')!) as PrezunicPromotion[]
				)
				...
				offers
					.filter(o => o.type && specialOffers.includes(o.type))
					.forEach(o => {
