...
export class MundialScraper extends MarketScraper {
	...
	protected override async _start() {
		...
		const [res] = await Promise.all([page.goto(URL), this.logger.info(`Acessando o site ${URL}`)])
		...
		await Promise.all([$popup_reject?.click(), this.logger.debug(`Rejeitando cookies...`)])
		let count = 1
		...
		do {
			...
			if ($button_show_more && (await isVisible($button_show_more))) {
				await Promise.all([$button_show_more.click(), this.logger.debug(`[${count++}] Carregando mais produtos...`)])
				...
				while (await isVisibleAndDisable()) {
					await Promise.all([sleep(1000), this.logger.debug(`Esperando o botão ser habilitado novamente...`)])
				}
			} else break
		} while (true)
	}
	...
	private async getProduct(
		...
		callback: (p: Product) => Promise<Product> = async p => p
		...
		let offer: Offer = { price: parseNumber(offerPrice), endDate: offerEndDate ? new Date(offerEndDate) : undefined }
			...
			for (let i = 0; i < promotionEntries.length; i++) {
				...
				if (!match && i < promotionEntries.length - 1) continue
					...
					await this.printAndThrowError(el, {
						...
						imageName: `promotion-unknown_${name.replaceAll(' ', '_')}`
					})
					return
				}
